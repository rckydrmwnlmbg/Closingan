...
describe('FonnteService - Provider Timeout Hardening', () => {
  ...
  it('should gracefully handle TimeoutError on generateQrCode and throw standard error', async () => {
    jest.spyOn(httpService, 'post').mockReturnValue(throwError(() => new TimeoutError()));

    await expect(fonnteService.generateQrCode('tenant-1')).rejects.toThrow(InternalServerErrorException);
  });

  it('should return failure result gracefully on timeout during sendMessage', async () => {
    jest.spyOn(httpService, 'post').mockReturnValue(throwError(() => new TimeoutError()));

    const result = await fonnteService.sendMessage({ tenantId: 'tenant-1', to: '123', message: 'test' });
    expect(result.success).toBe(false);
    expect(result.error).toBe('Fonnte API timeout');
  });

  it('should succeed sendMessage if no timeout', async () => {
    jest.spyOn(httpService, 'post').mockReturnValue(of({ data: { status: true, id: ['msg1'] } }) as any);
